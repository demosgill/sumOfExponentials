...
def fitNlinears(data, gammaFix=None):
    ...
    if gammaFix is None:
        ...
        return [pars[0], gammaFix, pars[1]], mfun
...
def createInitialValues(data, numpts0=numpts0, gammaFix=None):
    ...
    if gammaFix is None:
        ...
        def cost(x):
            return fitLinears(data, [x[0], gammaFix, x[1]])[-1]
        PTS = [GRID_G1, GRID_G3]
...
def hessianForGivenGammaTwo(pars, data, gammaFix):

    # Getting the Hessian: |profile_gammaOne(pars, data, gamma_one|
    f = lambda x: profile_gammaTwo(x, data, gammaFix)

    # Get Hessian
    Hfun = ndt.Hessian(f)
    H = Hfun(np.array([pars[0], pars[1], pars[2], pars[3], pars[5]]))
...
# ---------------------------------------------------------------
def profile_gammaTwo(pars, data, gamma_fix, simulate=False):

    """ enter a fix value for gamma_one """

    A,B,C = pars[0], pars[1], pars[2]
    gamma_one,gamma_two,gamma_three = pars[3], gamma_fix, pars[4]
...
def calc_scores_soe(pars, data, fixedGamma):
    ...
    small_pars_set = np.array([pars[0], pars[1], pars[2], pars[3], pars[5]])
    ...
    for i in xrange(len(small_pars_set)):
        ...
        _, logliksplus = profile_gammaTwo(small_pars_set + delta,
                                                   data, fixedGamma, simulate=True)
        _, loglikminus = profile_gammaTwo(small_pars_set - delta,
                                                    data,fixedGamma, simulate=True)
...
def modLikStep(ind, pars, data, X_hat, s_tc):
    ...
    H, X = hessianForGivenGammaTwo(pars, data, pars[4])
...
def calcModLogLik(pars, data, unbiased_s=True, job_pool=None):
    ...
    logLp, lpHat, s_tc = profilingGammaTwo(data)
    if unbiased_s:
        p = 5.
        s_tc = s_tc * float(len(data)) / float(len(data) - p)

    # X for MLE
    H, X_hat = hessianForGivenGammaTwo(pars, data, lpHat[3])
